...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Query query = session.createQuery("select f from Flight f join f.airplane a " +
                "where f.origin.country like :country and a.capacity >500 order by f.flightnr");
        ...
        List<Airline> airlines = session.createQuery(
                "select distinct a from Airline a join a.flights f join f.airplane ap where ap.model like 'A380'").list();
        ...
        flights = session.createQuery(
                "select f from Flight f join f.airplane ap join f.airline a where ap.model like '747' and a.name not like 'Star Alliance'").list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
            System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
        ...
        query = session.createQuery(
                "select f from Flight f where f.departureTime < :deptime and f.departureDate = :depdate");
        query.setParameter("deptime", Time.valueOf("12:00:00"));
        query.setParameter("depdate", Date.valueOf("2009-08-07"));
        flights = query.list();
        System.out.printf("%-9s%-31s%-31s\n", "Flight:", "Departs:",
                "Arrives:");
        for (Flight flight : flights) {
